...
class VoipSettings(models.Model):
    ...
    inactivity_time = fields.Integer(string="Inactivity Time (Minutes)",
                                     help="The amount of minutes before the user is considered offline", required=True)
    record_calls = fields.Boolean(string="Record SIP Calls")

    codec_id = fields.Many2one('voip.codec', string="Default Codec",
                               help="When a call is accepted it specifies to use this codec, all media should be pre-transcoded in this codec ready to be streamed",
                               required=True)
